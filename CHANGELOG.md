...
### Fixed
- **No more silent loss of local edits — real 3-way merge (Phase 3).** Until now, opening a markdown file whose local disk content had diverged from the server's Y.Text (offline edits, edits via another app, edits while disconnected) silently replaced your local content with the server version — no warning, no recovery. The proper merge is now implemented using the proven "DiskBuffer-mediated three-way merge with diff-match-patch deltas" pattern (the same approach Relay uses). A per-file snapshot of the last-known-in-sync content is persisted in a dedicated IndexedDB store keyed by the file's stable UUID; on bind, if disk and Y.Text differ, we compute the local edits since that base and replay them onto the shared Y.Text so BOTH sides' edits survive. When there's no base to merge against (first-ever open), we fall back to an additive union (never deletes) plus a Notice so you can eyeball the result. The editor buffer is never cleared on any path, including error handling — losing user text is no longer possible by construction. The merge base is dropped when a file is deleted, and deliberately survives the "Wipe local cache" command (so wiping cache doesn't reintroduce the data-loss window). Note: the merge base store is NOT cleared by "Wipe local cache" on purpose.
- **"Friend invisible everywhere" — root cause fixed.** The recurring complaint where a peer's cursor vanished from all files after they switched files once. Root cause was two independent code paths writing the local awareness (presence) state and racing: the editor binding called `removeAwarenessStates` on file switch, which deleted the local state entirely, after which the cursor-position writer silently no-op'd (you can't add a field to a null state) — so the peer advertised nothing and showed as gone until their next file switch. Underlying that was a wrong assumption baked into the code: it believed Obsidian destroys the editor on file switch, when Obsidian actually REUSES the same editor. Fix: `LocalPresenceController` is now the single, exclusive writer of local awareness state; the editor binding no longer touches it on switch (only `TextSession.destroy` removes a peer's presence, for genuine teardown). The cursor writer is also null-safe — it re-establishes identity if presence's broadcast hasn't landed yet. A peer going invisible is no longer reachable by any switch sequence.
- **Auth-failure reconnect storm stopped.** When the JWT token expired or was rejected, the plugin showed one Notice then silently hammered the server with reconnect attempts forever (the WebSocket layer auto-reconnects and nothing inspected the rejection). Now an auth failure stops the reconnect loop, shows a persistent "⚠ collab auth failed — update token" indicator in the status bar, and auto-recovers the moment you edit the token in settings or run "Reconnect to server".
- **Binary downloads no longer strand on a first miss.** A peer's image whose manifest entry arrived before its bytes finished uploading (very common — the manifest update is tiny, the blob is megabytes) returned 404 once and was never retried, leaving a broken image until a manual reconnect. Downloads now retry 404 / network / 5xx with bounded backoff (1s→8s ×5), and any still-unresolved binaries are parked and re-probed every 30s plus on every reconnect, so a late-landing blob resolves on its own.
- **Downloaded binary bytes are verified against their content hash.** Closes the last gap in the content-addressed integrity model — a corrupted or wrong blob from the server is now rejected (never written to the vault) instead of trusted blindly. A hash mismatch is treated as poison and not retried in a loop.
- **`connect()` self-heals a dead socket.** Previously it early-returned whenever a socket object existed, even if the underlying connection was dead — so auto-connect and "Reconnect" could leave you stuck offline. It now rebuilds a dead socket instead of no-opping, and cooperates with the auth-failure stop so the token-fix recovery path reconnects cleanly.

### Notes
- These are all client-side fixes. The server is unchanged; `MIN_CLIENT_VERSION` stays at 2.0.0 so 2.0.x and 2.1.0 peers interoperate (older peers just lack the fixes). Upgrade all peers when convenient.
- Deferred to a later release (from the same review, lower severity): folder-rename child re-pathing, rename-by-UUID identity check, manifest entry validation hardening, trash-list virtualization, and the connection-pool / mobile-background work (Phase 7).
